...
books_trimmed %>% 
  ...
  arrange(desc(median_author_rating))
```
...
books_trimmed %>% 
  group_by(authors) %>% 
  mutate(median_author_rating = median(average_rating)) 
```
...
books_trimmed <- books_trimmed %>% 
  group_by(authors) %>% 
  mutate(median(average_rating)) 

```

```{r}
books_trimmed <- books_trimmed %>% 
  select(authors, ratings_count, `median(average_rating)`) 
```


```{r}
books_trimmed <-  books_trimmed %>% 
 rename("median_rating" =  "median(average_rating)" )
...
books_trimmed_above_9 <- books_trimmed_above_9 %>% 
  group_by(authors) %>% 
  summarise(total_count = sum(ratings_count), median_rating = median(median_rating))
```
...
books_trimmed_JK_and_JRR <- books_trimmed_above_9 %>% 
  filter(grepl("J.K. Rowling|J.R.R. Tolkien", authors))
```
